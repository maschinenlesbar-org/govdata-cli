...
The skills are published as the `govdata` plugin in the
...
/plugin install govdata@maschinenlesbar
```

The first command registers the marketplace (once, for all maschinenlesbar.org
plugins); the second installs the `govdata` plugin, which bundles all three skills.
